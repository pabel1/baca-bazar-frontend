    ...
    <>
      {data && (
        <div className="w-full">
          <div
            className="w-full flex gap-6 items-center lg:justify-between transition group ">
            <NavLink
              to={data?.link}
              className="py-2 flex gap-2 items-center">
              <data.Icon />
              <span className="block text-[0.9rem] md:hidden lg:block">{data.title}</span>
            </NavLink>
            {data?.subNav ? (
              <button
...
{/* for sub menu */}
          ...
          >
            {data?.subNav?.map((item, i) => {
              return (
                <li
                  className="list-none mb-4 decoration-slate-200"
                  key={i}
                >
                  <NavLink
                    to={item.link}
                    className="flex gap-x-2 items-center ml-6">
