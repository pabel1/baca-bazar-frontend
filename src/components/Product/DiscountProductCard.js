...
const DiscountProductCard = ({ data }) => {
    ...
    <>
      {data && (
        <div
          ...
          flex flex-col items-center justify-between rounded-md cursor-pointer relative"
        >
         <div className="absolute top-2 left-2 bg-secondary w-9 h-9 rounded-full z-[100] text-bgColor text-[0.7rem] flex justify-center items-center">{data.discountInPercent}%</div>
          <img
          ...
          <div className="w-full  flex flex-col  justify-between mb-2  ">
            ...
            >
              {" "}
              <p className="w-full h-[35%] text-[0.9rem] white text-left text-lightText  hover:text-primaryColor cursor-pointer">
                {data.productname.slice(0, 50)}
              </p>
            </Link> */}
            <p className="w-full h-[20%] text-left text-xl text-yellow-500 flex  items-center justify-between">
              &#2547; {parseInt(data.price) - parseInt(data.discountPrice)}
