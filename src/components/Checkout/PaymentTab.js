...
import { CartContext, PaymentInfoContext, ShippingCost } from "../../context";
...
const PaymentTab = () => {
  ...
  const handlePaymentInfo = (e) => {
    ...
    if (isValid) {
      setPaymentInfo({ paymentMethod, transactionId });
  ...
  const handleCuponAdd = async (e) => {
    ...
    if (res && res.status === 200) {
      ...
      if (res.data.cupon.discountType === "Tk") {
        subtotal = subtotal - parseInt(res.data.cupon.discountPercent);
      }
      if (res.data.cupon.discountType === "%") {
        let tk = (subtotal * parseInt(res.data.cupon.discountPercent)) / 100;
      ...
      <div className="grid grid-cols-2 gap-x-5  md:flex justify-between gap-4  mb-8">
        <div
          onClick={() => {
            ...
            setPaymentInfo({ paymentMethod: "cashOnDelivery" });
          }}
          className="w-32 h- cursor-pointer flex justify-center items-center border border-secondary rounded-xl"
        >
          <img
            src={cashondelivery}
            alt=""
        ...
        <div
          ...
          className="w-32 h-20 flex cursor-pointer justify-center items-center border border-secondary rounded-xl"
        >
