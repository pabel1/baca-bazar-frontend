...
const ProductPopUp = () => {
  ...
  return (
    <div
      className={`w-screen h-screen p-10 bg-[rgba(255,255,255,0.8)] ${
        productPopUpShow.show ? "flex" : "hidden"
      } items-center justify-center fixed top-0 left-0 right-0 bottom-0 z-[10000]`}
    >
      <div
        className=" container md:w-[70%] md:h-[80vh] p-6 flex flex-col justify-center items-center bg-white 
    shadow-2xl rounded-lg shadow-[rgba(0,0,0,0.5)]"
      >
        {/* product details  */}
        <div className="relative flex items-center gap-3 flex-wrap  md:flex-nowrap px-5 rounded-sm ">
          <div className="basis-[80%] md:basis-[40%] mx-auto ">
            <img src={state.image} alt="" className=" w-full" />
          </div>
          <div className="container relative basis-[80%] mx-auto md:basis-[60%] bg-gray-100 px-2 py-3 space-y-4">
            ...
            <div className="flex items-center justify-between border-b-2 pb-3">
              ...
              {state.stock < 1 ? (
                ...
                <p className=" text-secondary font-semibold text-sm"> In Stock</p>
              )}
            </div>
            <div
      ...
      scrollbar-thumb-rounded-full scrollbar-track-rounded-full"
          ...
          {state.discountInPercent && <div className="absolute top-2 left-2 bg-secondary w-20 h-20 rounded-full z-[100] text-bgColor text-[1.2rem] flex flex-col justify-center items-center">{state.discountInPercent}%<span className="">Off</span></div>}
